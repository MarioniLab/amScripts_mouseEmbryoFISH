...
estimated.counts.mean = lapply(mapping.pca, function(x){
...
estimated.counts.weightedMean = lapply(mapping.pca, function(x){
  cells = x$cells.mapped
  cells = cells[1:K]
  idx = order(cells)
  cells = cells[idx]
...
colnames(estimated.counts.weightedMean) = names(mapping.pca)
rownames(estimated.counts.weightedMean) = rownames(sce.atlas)
saveRDS(estimated.counts.weightedMean, file = paste0( root.dir, "data/8_5/stitching/pca_K25_weightedMean.rds"))
