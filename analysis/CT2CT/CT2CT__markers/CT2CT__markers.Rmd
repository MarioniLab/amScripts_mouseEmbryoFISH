      ...
      corr.stat = lapply(unique(current.meta$embryo), function(x){
        ...
        for (i in 1:nrow(corr.stat.embryo)){
          if ( sum(rownames(current.counts.oneEmbryo) == corr.stat.embryo$Gene[i]) == 1) {
            ...
            stat.weird = data.frame(CT1 = x1, CT2 = x2, gene.toInvestigate = corr.stat.embryo$Gene[i])
            save.name = paste0("toCheck_stitched_", x1, "_", x2, "_", corr.stat.embryo$Gene[i], ".tab")
            write.table(stat.weird, file = paste0(root.dir, "data/8_5/CT2CT/", save.name), row.names = F, col.names = T, sep = "\t")
          }
      ...
      corr.stat = lapply(unique(current.meta$embryo), function(x){
        ...
        for (i in 1:nrow(corr.stat.embryo)){
          if ( sum(rownames(current.counts.oneEmbryo) == corr.stat.embryo$Gene[i]) == 1) {
            ...
            stat.weird = data.frame(CT1 = x1, CT2 = x2, gene.toInvestigate = corr.stat.embryo$Gene[i])
            save.name = paste0("toCheck_seq_", x1, "_", x2, "_", corr.stat.embryo$Gene[i], ".tab")
            write.table(stat.weird, file = paste0(root.dir, "data/8_5/CT2CT/", save.name), row.names = F, col.names = T, sep = "\t")
