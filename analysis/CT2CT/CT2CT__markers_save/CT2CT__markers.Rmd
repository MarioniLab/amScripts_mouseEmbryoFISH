...
counts.stitched = readRDS(paste0( root.dir, "data/8_5/stitching/pca_K10_mean__nonZero.rds") )
#counts.stitched = readRDS(paste0( root.dir, "data/8_5/stitching/pca_K10_mean__toCheck.rds") )
    ...
    if (!x1 == x2){
      ...
      corr.stat = lapply(unique(current.meta$embryo), function(x){
        ...
        for (i in 1:nrow(corr.stat.embryo)){
          if ( sum(rownames(current.counts.oneEmbryo) == corr.stat.embryo$Gene[i]) == 1) {
      ...
      save.name.x1 = str_replace_all(x1   , " ", "_")
      save.name.x1 = str_replace_all(save.name.x1   , "/", "_")
      save.name.x2 = str_replace_all(x2   , " ", "_")
      save.name.x2 = str_replace_all(save.name.x2   , "/", "_")
      saveRDS(corr.stat, file = paste0( root.dir, "data/8_5/CT2CT/stitched_markers_" ,save.name.x1 , "__",save.name.x2, ".rds"))
