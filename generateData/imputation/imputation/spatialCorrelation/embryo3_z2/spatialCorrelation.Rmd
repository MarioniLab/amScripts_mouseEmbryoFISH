...
meta = meta[meta$iterMNN_stable_mapping_quality == "Good" & !meta$iterMNN_stable %in% CTs.2discard & meta$embryo_z == "embryo3_2", ]
...
spatialCorrelation = function(n.neigh){
  ...
  spat.corr = bplapply(genes, function(current.gene){
    return( cor(imputation[rownames(imputation) == current.gene,], spatial.imputation[rownames(spatial.imputation) == current.gene,], method = "pearson", use="complete.obs") )
  }, BPPARAM = mcparam)
  return(spat.corr)
}
...
saveRDS(spatial.correlation, file =  paste0( root.dir, "data/8_5/stitching/imputation/spatialCorrelation_embryo3z2.rds"))
