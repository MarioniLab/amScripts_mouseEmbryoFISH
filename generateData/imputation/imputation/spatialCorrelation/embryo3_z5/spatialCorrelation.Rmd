...
meta = meta[meta$embryo_z == "embryo3_5" , ]
sce.seq = sce.seq[, colnames(sce.seq) %in% meta$uniqueID ]

# load mappings
imputation = readRDS(paste0( root.dir, "data/8_5/stitching/imputation/imputation_seq2atlas_logcounts_embryo3z5.rds"))
...
spatialCorrelation = function(n.neigh){
  ...
  spat.corr = bplapply(genes, function(current.gene){
    out = data.frame(gene = current.gene, spat.corr = cor(imputation[rownames(imputation) == current.gene,], spatial.imputation[rownames(spatial.imputation) == current.gene,], method = "pearson", use="complete.obs"))
    return( out )
  }, BPPARAM = mcparam)
  spat.corr = do.call(rbind, spat.corr)
  return(spat.corr)
}
...
saveRDS(spatial.correlation, file =  paste0( root.dir, "data/8_5/stitching/imputation/spatialCorrelation/spatialCorrelation_embryo3z5.rds"))
