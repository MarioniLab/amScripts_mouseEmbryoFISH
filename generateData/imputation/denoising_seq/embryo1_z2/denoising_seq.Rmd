...
counts.denoised = lapply( n.neigh.grid , function(i) {
  ...
  counts.denoised.median = denoisingCounts(counts.cosine.seq, i, function(x){ median(x) }, mcparam)  
  ...
  current.counts.denoised = list(mean = counts.denoised.mean, median = counts.denoised.mean, 
                                 min = counts.denoised.min, max = counts.denoised.max, 
                                 q25 = counts.denoised.q25, q75 = counts.denoised.q75)
  return(current.counts.denoised)
})
names(counts.denoised) = paste0("nn_", n.neigh.grid)  
saveRDS(counts.denoised, file = paste0(root.dir, "data/8_5/stitching/seq_denoised_embryo1z2.rds"))
