...
spat.coherence = readRDS(paste0(root.dir, "data/8_5/stitching/seq_spatialCoherency_withinZ.rds"))
spat.coherence = spat.coherence$n.neigh_1
...
Spatially coherent cells = spat. coherence > 0.7 (dist = 1, within z) or can not be asigned

```{r gene-score-spatCoherent, message=FALSE, warning=FALSE}



spat.coherent.thresh = 0.7 

unq.embryo_z = unique(meta$embryo_z)
gene.score.allEmbryoZ = bplapply(unq.embryo_z, function(x){
  gene.score.oneEmbryoZ = lapply(genes, function(current.gene){
    current.atlas = counts.atlas[rownames(counts.atlas) == current.gene,]
    current.spatCoherence = spat.coherence[rownames(spat.coherence) == current.gene,]
    
    cells.spatCoherent = names(current.spatCoherence[is.na(current.spatCoherence) | current.spatCoherence >= spat.coherent.thresh])
    
    # closest cell
    current.mapping = mapping[rownames(mapping) == paste0( current.gene, "_1"), colnames(mapping) %in% meta$uniqueID[meta$embryo_z == x]]
    # select only spat coherent cells
    current.mapping = current.mapping[names(current.mapping) %in% cells.spatCoherent]
    ...
    current.mapping = mapping[grepl(paste0( current.gene, "_"), rownames(mapping)), colnames(mapping) %in% meta$uniqueID[meta$embryo_z == x]]
    # select only spat coherent cells
    current.mapping = current.mapping[, colnames(current.mapping) %in% cells.spatCoherent]
...
gene.score.allEmbryoZ = do.call(rbind, gene.score.allEmbryoZ)
saveRDS(gene.score.allEmbryoZ, file =  paste0( root.dir, "data/8_5/stitching/geneScore_spatCoherent_performance.rds"))
