...
sce.atlas = sce.atlas[rownames(sce.seq), sce.atlas$sample == 29]
...
performance.seq = bplapply(genes, function(current.gene){
  ...
  if (length(current.noise.cv) > 0){
    ...
    cv.norm.imputed = cv.norm.imputed / NA
  }
...
saveRDS(performance.seq, file =  paste0( root.dir, "data/8_5/stitching/performance_atlas2atlas_s29.rds"))
