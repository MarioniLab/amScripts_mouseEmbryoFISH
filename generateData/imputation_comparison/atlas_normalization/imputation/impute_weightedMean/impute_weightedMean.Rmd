...
genes = genes$external_gene_name[genes$experiment == "sm"]
sce.atlas = sce.atlas[rownames(sce.atlas) %in% genes, ]
counts.atlas = logcounts(sce.atlas)

# get mapping
mapping = readRDS(paste0( root.dir, "data/8_5/mapping_atlas_to_atlas.rds"))
...
estimated.counts = lapply(1:length(mapping), function(y){
  estimated.counts.per_nPC = lapply(n.neigh, function(current.n){
    ...
    saveRDS(estimated.counts.per_nPC.per_nn, 
            file =  paste0( root.dir, "data/8_5/stitching/compareMetrics/atlas_onto_atlas/nn_", current.n, "__n", names(mapping)[y], "__weightedMean.rds"))
  })
})
